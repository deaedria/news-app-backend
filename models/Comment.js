...
const commentModel = {
        ...
        return new Promise((resolve, reject) => {
            pg.query(`SELECT * from comments LIMIT ${limit} OFFSET ${(page-1) * limit}`, (err, result) => {
                if (result.rows.length < 0) {
                    reject(formError("Comment not found", 400))
                }
                if (!err) {
                    resolve(formSuccess("Get all Comments success", 200, result.rows));
                } else {
                    reject(formError("Get all Comments failed", 500));
                }
            }
            );
        });
                    ...
                    pg.query(`SELECT username FROM users WHERE id=${sender_id}`, (err1, res) => {
                        if(!err1){
                            ...
                            pg.query(`INSERT INTO notification_list(user_id,is_comment,comment_from,created_at,notif)
                                ...
                                if (!error) {
                                    resolve(formSuccess("Add comment success", 200, result.rows[0]));
                                } else {
                                    reject(formError("Add comment Failed", 500));
                                }
                            });
                        }else{
                            reject(formError("Add comment Failed", 500));
            ...
            pg.query(`SELECT * FROM comments WHERE id=${request}`, (err, result) => {
                if (!err) {
                    if (result.rows.length < 1) {
                        ...
                        resolve(formSuccess("Get comment success", 200, result.rows[0]));
                    }
                } else {
                    reject(formError("Get comment failed", 500));
                }
            });
        });
    },
    ...
    getCommentListByUserId: (request) => {
        return new Promise((resolve, reject) => {
            ...
            FROM comments as c JOIN users as u ON c.sender_id = u.id WHERE c.receiver_id = ${request} ORDER BY created_at DESC`, (err, result) => {
                if (!err) {
                    if (result.rows.length < 1) {
                        reject(formError("Comment list by user id not found", 400));
                    } else {
                        resolve(formSuccess("Get comment list by user id success", 200, result.rows));
                    }
                } else {
                    reject(formError("Get comment list by user id failed", 500));
                }
            });
        });
    },
    ...
    getCommentListByArticleId: (request) => {
        return new Promise((resolve, reject) => {
            pg.query(`SELECT c.article_id, a.article_title, a.author_id, c.receiver_id, c.sender_id, u.name, u.username, u.photo_profile,
                ...
                if (!err) {
                    if (result.rows.length < 1) {
                        reject(formError("Comment list by article id not found", 400));
                    } else {
                        resolve(formSuccess("Get comment list by article id success", 200, result.rows));
                    }
                } else {
                    reject(formError("Get comment list by article id failed", 500));
                }
            });
        });
    },
                ...
                if (!error) {
                    if (res.rows.length < 1) {
                        ...
                        pg.query(`DELETE FROM comments WHERE id=${request} RETURNING *`, (err, result) => {
                            if (!err) {
                                resolve(formSuccess('Delete comment success', 200, result.rows[0]));
                            } else {
                                reject(formError('Delete comment failed', 500));
                            }
                        });
                    }
                } else {
    ...
    //                 reject(formError("category id not found", 400));
    //             }
    //             if (!error) {
